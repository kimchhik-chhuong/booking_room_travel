        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 bg-white p-6 rounded-lg shadow">
                ...
                <div class="space-y-3 max-h-96 overflow-y-auto scrollbar-thin" id="conversationList">
                    @php
                    $conversations = [
                        ['id' => 'EH', 'name' => Auth::user()->name ?? 'User1', 'message' => 'We are pleased to inform you about the booking confirmation...', 'time' => '10:24 AM', 'unread' => true, 'avatar' => 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name ?? 'User1') . '&background=random&size=40'],
                        ['id' => 'GC', 'name' => 'User2', 'message' => 'We have updated our contract terms and conditions...', 'time' => '2:30 PM', 'unread' => true, 'avatar' => 'https://ui-avatars.com/api/?name=User2&background=random&size=40'],
                        ['id' => 'KU', 'name' => 'User3', 'message' => 'Hi, I have some questions about the Venice package...', 'time' => '5:45 AM', 'unread' => false, 'avatar' => 'https://ui-avatars.com/api/?name=User3&background=random&size=40'],
                        ['id' => 'OF', 'name' => 'User4', 'message' => 'Hello, I had an amazing time on the Tokyo tour...', 'time' => '10:15 AM', 'unread' => false, 'avatar' => 'https://ui-avatars.com/api/?name=User4&background=random&size=40'],
                        ['id' => 'MJ', 'name' => 'User5', 'message' => 'Can you send more details about the safari package?', 'time' => '7:24 PM', 'unread' => false, 'avatar' => 'https://ui-avatars.com/api/?name=User5&background=random&size=40'],
                    ];
                    @endphp
            ...
            <div class="lg:col-span-2 bg-white rounded-lg shadow flex flex-col h-[600px]">
                ...
                <div class="flex items-center justify-between p-6 border-b border-gray-200" id="chatHeader">
                    <div class="flex items-center space-x-4">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($conversations[0]['name']) }}&background=random&size=40" alt="{{ $conversations[0]['name'] }}" class="w-12 h-12 rounded-xl shadow">
                        <div>
                            <p class="text-lg font-bold text-gray-800" id="chatName">{{ $conversations[0]['name'] }}</p>
                            <p class="text-sm text-green-600 flex items-center">
                                <span class="w-2 h-2 bg-green-500 rounded-full mr-2"></span>Online
                            </p>
                        </div>
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-6" id="chatMessages" role="region" aria-live="polite">
                    <!-- Incoming Message (Example) -->
                    <div class="flex items-end space-x-3">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($conversations[0]['name']) }}&background=random&size=32" alt="{{ $conversations[0]['name'] }}" class="w-8 h-8 rounded-xl">
                        <div class="bg-gray-100 p-3 rounded-xl rounded-bl-md max-w-xs lg:max-w-md">
                            <p class="text-sm text-gray-800">We are pleased to inform you that your booking for the "Venice Dreams" package has been confirmed for June 25-30.</p>
                            <p class="text-xs text-gray-500 mt-1">10:24 AM</p>
                        </div>
...
@push('scripts')
<script>
    // Conversations data (updated to dynamically reflect user chats)
    let conversations = [
        { id: 'EH', name: '{{ Auth::user()->name ?? 'User1' }}', message: 'We are pleased to inform you about the booking confirmation...', time: '10:24 AM', unread: true, avatar: 'https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name ?? 'User1') }}&background=random&size=40' },
        { id: 'GC', name: 'User2', message: 'We have updated our contract terms and conditions...', time: '2:30 PM', unread: true, avatar: 'https://ui-avatars.com/api/?name=User2&background=random&size=40' },
        { id: 'KU', name: 'User3', message: 'Hi, I have some questions about the Venice package...', time: '5:45 AM', unread: false, avatar: 'https://ui-avatars.com/api/?name=User3&background=random&size=40' },
    ...
    function sendTextMessage(text) {
        ...
        // Ensure the current user is reflected in the conversation list
        if (!conversations.some(conv => conv.id === currentConversation.id)) {
            conversations.push({ id: currentConversation.id, name: auth().user.name, message: text, time: time, unread: true, avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(auth().user.name)}&background=random&size=40` });
            saveConversations();
            updateConversationList();
        }
    }
